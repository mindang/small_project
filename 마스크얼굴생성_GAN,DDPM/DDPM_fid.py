...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    covmean, _ = linalg.sqrtm(sigma1.dot(sigma2), disp=False)       # 공분산 행렬의 곱의 제곱근
    if not np.isfinite(covmean).all():                              # 무한값이면 입실론 추가후 계산
    ...
    if np.iscomplexobj(covmean):                                     # 복소수라면 허수가 있는지 판단, 허수라면 오류
...
def calculate_fid(images1, use_multiprocessing, batch_size , with_mask):
    ...
    if with_mask:
        print('with_mask ------ ',end=' ')
        mu2 = np.load('Test_mu_w.npy')
        sigma2 = np.load('Test_sigma_w.npy')
    else:
        print('without_mask ------ ',end=' ')
        mu2 = np.load('Test_mu_wo.npy')
        sigma2 = np.load('Test_sigma_wo.npy')

    #mu1, sigma1 = np.array(mu1) , np.array(sigma1) 
    #mu2, sigma2 = np.array(mu2) , np.array(sigma2) 
...
def fid_return(image_path,batch_size,with_mask=True):
    images1 = load_images(image_path)
    fid_value = calculate_fid(images1, False, batch_size , with_mask = with_mask)
    return fid_value