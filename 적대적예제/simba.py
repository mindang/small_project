...
class SimBA:
    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        for k in range(max_iters):
            dim = indices[k]
            #print(x.device,images_batch.device)
            x = x.to(self.device)
            ...
            remaining_indices = remaining_indices.to(self.device)
            if k > 0:
                succs[:, k] = ~remaining
            ...
            left_probs = self.get_probs(adv, labels_batch[remaining_indices]).to(self.device)
            queries_k = torch.zeros(batch_size).to(self.device)
            # increase query count for all images
            queries_k[remaining_indices] += 1
            prev_probs.to(self.device)
            #print('hi')
            #print(prev_probs.device,remaining_indices.device)
            if targeted:
            ...
            right_probs = self.get_probs(adv, labels_batch[remaining_indices]).to(self.device)
            if targeted:
